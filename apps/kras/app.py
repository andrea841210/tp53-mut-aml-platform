...
# Streamlit app v1.5 — Chart view cap (Top 50), median aggregation, TCGA direct filter, audit tools
...
DATA_DIR = Path("data")
FILES = {
    "CCLE": DATA_DIR / "CCLE_mutation_22Q2.csv",   # or KRAS geneset wide file
...
def load_models(path: Path) -> pd.DataFrame:
    ...
    # --- Oncotree → TCGA mapping fallback ---
...
def mutated_depmap_ids(ccle: pd.DataFrame, gene: str) -> Set[str]:
    # Long format: DepMap_ID, Hugo_Symbol
...
models = load_models(FILES["MODEL"])  # provides DepMap_ID, TCGA_Classification (or Oncotree-mapped)
ccle = load_ccle(FILES["CCLE"])   # long or wide
...
# Collapse duplicates by median per (Drug, DepMap_ID) or (Drug, CellLine)
